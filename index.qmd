...
```{ojs}
viewof n = Inputs.range([64, 512], { value: 256, step: 64, label: "Image size (n×n)" })
viewof mu = Inputs.range([-2, 2], { value: 0, step: 0.01, label: "μ (mean)" })
viewof sigma = Inputs.range([0.1, 2.5], { value: 1, step: 0.01, label: "σ (std dev)" })
viewof clip = Inputs.range([1, 6], { value: 3, step: 0.1, label: "Display clip = ±(clip·σ)" })
viewof seed = Inputs.range([0, 9999], { value: 0, step: 1, label: "Seed" })
viewof regen = Inputs.button("Regenerate (same params)")
...
md`当前：n=${n}, μ=${mu.toFixed(2)}, σ=${sigma.toFixed(2)}（显示按 ±${clip.toFixed(1)}σ 截断映射到灰度）`


// 2. 复杂的计算和绘图逻辑，必须用 { } 包裹起来作为一个整体
{
  noiseImage = {
// 让按钮 regen 也能触发重画（即使参数没变）
regen

rand = prng(seed)               // 固定 seed，可复现
...
// 映射：val ~ N(mu, sigma^2)
// 灰度 = 128 + (val-mu)/(clip*sigma) * 128，然后截断到 [0,255]
denom = clip * sigma

i = 0
for (p = 0; p < n * n; p++) {
val = mu + sigma * randn(rand)
g = 128 + (val - mu) / denom * 128
