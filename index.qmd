...
```{ojs}
viewof n_2d = Inputs.range([64, 512], { value: 256, step: 64, label: "n×n" })
viewof mu_2d = Inputs.range([-2, 2], { value: 0, step: 0.01, label: "μ (mean)" })
viewof sigma_2d = Inputs.range([0.1, 2.5], { value: 1, step: 0.01, label: "σ (std dev)" })
viewof clip_2d = Inputs.range([1, 6], { value: 3, step: 0.1, label: "Display clip ±(clip·σ)" })
viewof seed_2d = Inputs.range([0, 9999], { value: 0, step: 1, label: "Seed" })
viewof regen_2d = Inputs.button("Regenerate")

md`n=${n_2d}, μ=${mu_2d.toFixed(2)}, σ=${sigma_2d.toFixed(2)}`

{
  // 定义核心绘图函数
  const makeNoiseCanvas = (n, mu, sigma, clip, seed) => {
    
    // 伪随机数生成器 (Mulberry32)
    const prng = (s) => () => {
      s |= 0; s = (s + 0x6D2B79F5) | 0;
      let t = Math.imul(s ^ (s >>> 15), 1 | s); // 【修复】添加 let
    ...
    const randn = () => {
      let u = rand(); // 【修复】添加 let
      let v = rand(); // 【修复】添加 let
      u = Math.max(u, 1e-12);
      return Math.sqrt(-2 * Math.log(u)) * Math.cos(2 * Math.PI * v);
    }

    // 创建 Canvas
    const c = document.createElement("canvas"); // 【修复】添加 const
    c.width = n; 
    c.height = n;
    ...
    const denom = clip * sigma;
    
    // 填充像素
    let i = 0; // 【修复】添加 let
    for (let p = 0; p < n * n; p++) { // 【修复】添加 let
      let val = mu + sigma * randn(); // 【修复】添加 let
      
      // 归一化并映射到 [0, 255]
      let g = 128 + (val - mu) / denom * 128; // 【修复】添加 let
    ...
    c.style.imageRendering = "pixelated"; // 让像素点更清晰
    
    return c;
  }

  // 调用函数并显示
  // regen_2d 只是为了触发刷新，不需要传入函数
  regen_2d; 
  return makeNoiseCanvas(n_2d, mu_2d, sigma_2d, clip_2d, seed_2d);
}
