...
```{ojs}
viewof n = Inputs.range([64, 512], { value: 256, step: 64, label: "n×n" })
...
md`n=${n}, μ=${mu.toFixed(2)}, σ=${sigma.toFixed(2)}, clip=${clip.toFixed(1)}, seed=${seed}`

{makeNoiseCanvas = (n, mu, sigma, clip, seed) => {
prng = (s) => () => {
s |= 0
s = (s + 0x6D2B79F5) | 0
...
noiseCanvas = { regen; makeNoiseCanvas(n, mu, sigma, clip, seed) }
